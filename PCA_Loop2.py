...
for index, row in loop_df.iterrows():
    ...
    # Explained Variance Plot (same as before)
    # ...
    ...
    for j, pc in enumerate(['PC1', 'PC2']):
        ...
        # Color bars based on unique_colors

        bar_colors = []
        for label in contributions_sorted.index:
            try:
                # Extract last 3 characters before "nm" and convert to int
                wavelength = int(label.split('_')[-1][:-2])
                color = unique_colors.get(wavelength, (0, 0, 0))
            except ValueError:
                color = (0, 0, 0)  # Default color if conversion to int fails
            bar_colors.append(color)

        axs[index, j + 1].bar(contributions_sorted.index, contributions_sorted.values, color=[tuple(np.array(c)/255) for c in bar_colors], width=0.25)
